...
- 이력 조회 및 재평가 기능 제공
- **지원 상태 필터**: 직군/포지션, 추천 결과, 지원 상태(지원완료/서류심사/합격/불합격)로 필터링
- **상태 배지 표시**: 면접 이력 카드에 지원 상태 배지 자동 표시
- **필터 초기화**: 한 번에 모든 필터를 초기화하는 기능
...
- **지원 상태 관리**: 지원완료(SUBMITTED) → 서류심사(DOCUMENT_REVIEW) → 인터뷰진행(INTERVIEW) → 합격/불합격 자동 전환
  - 면접 스튜디오에서 에이전트 실행 완료 시 자동으로 서류심사 상태로 변경
  - 관리자가 면접 이력에서 “인터뷰 진행” 버튼을 누르면 자동으로 인터뷰 진행 단계로 전환되며, 지원자 화면에서도 동일하게 표시
